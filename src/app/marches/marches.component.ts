...
export class MarchesComponent implements OnInit {
  ...
  addMarche(newMarche: Marche) {
    this.marcheservice.Addmarche(newMarche).subscribe((result) => {
      this.chargeMarche();
      console.log(result);
      this.toast.success({detail:'Success',summary:'This is Success', sticky:true,position:'bl'})
    });
  }
  ...
  deleteMarche(m: Marche) {
    this.marcheservice.Deletemarches(m.id).subscribe((result) => {
      this.chargeMarche()
      console.log(result)
      this.toast.success({detail:'Success',summary:'This is Success', sticky:true,position:'br'})
    });
  }
